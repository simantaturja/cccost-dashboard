...
function parseSession(text, { sessionId, project }) {
  ...
  const toolNameById = new Map(); // tool_use_id -> tool name, to attribute errors
    ...
    if (Array.isArray(content)) {
      for (const block of content) {
        if (obj.type === 'assistant' && block.type === 'tool_use') {
          ...
          if (block.id) { seenToolUse.add(block.id); toolNameById.set(block.id, block.name); }
          waste.toolCallCount++;
          ...
          } else if (fp && (block.name === 'Edit' || block.name === 'Write')) {
          ...
          // tool, so that earlier failure did cost extra quota. Count it now.
          const retryIdx = pendingErrors.findIndex((e) => e.name === block.name);
          if (retryIdx !== -1) {
            const { name, reason } = pendingErrors[retryIdx];
          ...
          pendingErrors.push({ toolUseId: block.tool_use_id, name, reason }); // waste counted only if retried
        }
      }
    }
