...
const { parseSession, mergeSessionAggregates, planFromAccount } = require('./core');
...
function loadConfig() {
  ...
  if (config?.subscriptionUSDPerMonth != null) return config;
  const detectedPlan = planFromAccount(readClaudeAccount());
  return detectedPlan ? { ...config, detectedPlan } : config;
}

// Claude Code keeps its global state in ~/.claude.json, or under CLAUDE_CONFIG_DIR if set.
// Only the non-secret `oauthAccount` profile is read.
function readClaudeAccount() {
  const dir = process.env.CLAUDE_CONFIG_DIR || os.homedir();
  try {
    return JSON.parse(fs.readFileSync(path.join(dir, '.claude.json'), 'utf8')).oauthAccount;
  } catch {
    return null;
  }
}
