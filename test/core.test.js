...
test('parseSession dedup keeps the LAST occurrence when duplicates differ (streaming partials)', () => {
  ...
  const complete = JSON.stringify({
    ...
    message: {
      id: 'msg_dup',
      model: 'claude-opus-4-8',
      usage: { input_tokens: 100, output_tokens: 378 },
    },
  });
...
const acme1 = parseSession(
  line({ ts: '2026-06-30T10:00:00.000Z', cwd: '/work/acme/app', id: 'd1', input: 1e6 }),
  { sessionId: 'a1', project: 'p' },
);
const acme2 = parseSession(
  line({ ts: '2026-07-01T10:00:00.000Z', cwd: '/work/acme/app', id: 'd2', input: 2e6 }),
  { sessionId: 'a2', project: 'p' },
);
const globex = parseSession(
  line({ ts: '2026-07-05T10:00:00.000Z', cwd: '/work/globex/x', id: 'c1', input: 4e6 }),
  { sessionId: 'gx', project: 'p' },
);
const personal = parseSession(
  line({ ts: '2026-07-02T10:00:00.000Z', cwd: '/home/me/thing', id: 'o1', input: 1e6 }),
  { sessionId: 'per', project: 'p' },
);
...
test('buildReport renders a total-only monthly summary', () => {
  const r = buildResponse([acme1, acme2, globex, personal], TEST_CONFIG);
  assert.strictEqual(
    buildReport(r.monthly, '2026-07', '2026-07-14'),
    [
  ...
  assert.strictEqual(
    buildReport(r.monthly, '2026-05', '2026-07-14'),
    [
...
test('mergeSessionAggregates sums subagentCostUSD from non-main files', () => {
  const main = Object.assign(parseSession(opusLine, { sessionId: 'sess-1', project: 'p' }), {
    isMain: true,
  });
  const sub1 = Object.assign(parseSession(fableLine, { sessionId: 'sess-1', project: 'p' }), {
    isMain: false,
  });
  const sub2 = Object.assign(parseSession(fableLine, { sessionId: 'sess-1', project: 'p' }), {
    isMain: false,
  });
  const merged = mergeSessionAggregates([main, sub1, sub2]);
  assert.ok(Math.abs(merged.subagentCostUSD - 0.001515 * 2) < 1e-9);
  // no subagents → 0
  const solo = mergeSessionAggregates([
    Object.assign(parseSession(opusLine, { sessionId: 's', project: 'p' }), { isMain: true }),
  ]);
...
test('advisor rule 1: low cache ratio fires below 0.5 with non-trivial volume, not at threshold', () => {
  ...
  const fire = fakeSession({
    sessionId: 'r1-fire',
    costUSD: 2,
    tokens: { ...emptyTok(), input: 190000, cacheRead: 10000 },
  });
  const thresh = fakeSession({
    sessionId: 'r1-thresh',
    costUSD: 2,
    tokens: { ...emptyTok(), input: 100000, cacheRead: 100000 },
  });
  const nocost = fakeSession({
    sessionId: 'r1-cost',
    costUSD: 0.5,
    tokens: { ...emptyTok(), input: 190000, cacheRead: 10000 },
  });
  const small = fakeSession({
    sessionId: 'r1-small',
    costUSD: 2,
    tokens: { ...emptyTok(), input: 19000, cacheRead: 1000 },
  });
...
test('advisor rule 2: any top-tier model on a short session flags; est saving = premiumCost * 0.7', () => {
  ...
  const fable = fakeSession({
    sessionId: 'r2-fable',
    costUSD: 5,
    messages: 10,
    models: fableModels,
  });
  const mythos = fakeSession({
    sessionId: 'r2-mythos',
    costUSD: 5,
    messages: 10,
    models: mythosModels,
...
test('advisor sorts by cost desc and caps at 25', () => {
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    many.push(
      fakeSession({ sessionId: `s${i}`, costUSD: i, tokens: { ...emptyTok(), input: 200000 } }),
    );
  }
...
test('waste: counts tool_use calls and attributes errored tool_result to its tool name, only when retried', () => {
  const text = [
    ...
    asstTools({
      id: 'm2',
      blocks: [{ type: 'tool_use', id: 't4', name: 'Bash', input: { command: 'boom' } }],
    }), // retry of t3
  ].join('\n');
...
test("waste: an unrelated later call of the same tool does not falsely confirm a different call's error", () => {
  const text = [
    asstTools({
      id: 'm1',
      blocks: [{ type: 'tool_use', id: 't1', name: 'Bash', input: { command: 'rm -rf /tmp/A' } }],
    }),
    userResults({
      blocks: [
        { type: 'tool_result', tool_use_id: 't1', is_error: true, content: 'File does not exist' },
      ],
    }),
    // A different, unrelated Bash command — not a retry of t1 — that also happens to error.
    asstTools({
      id: 'm2',
      blocks: [{ type: 'tool_use', id: 't2', name: 'Bash', input: { command: 'curl https://x' } }],
    }),
    userResults({
      blocks: [
        { type: 'tool_result', tool_use_id: 't2', is_error: true, content: 'connection refused' },
      ],
    }),
  ].join('\n');
      ...
      blocks: [
        {
          ...
          id: 't2',
          name: 'WebFetch',
          input: { url: 'https://totally-different.example' },
        },
      ],
    }),
...
test('waste: a genuine WebFetch retry of the SAME url is still counted (url is now a target)', () => {
  const text = [
    ...
    asstTools({
      id: 'm2',
      blocks: [
        { type: 'tool_use', id: 't2', name: 'WebFetch', input: { url: 'https://a.example' } },
      ],
    }), // retry
  ].join('\n');
...
test('waste: an errored tool call with no later retry of the same tool is not counted', () => {
  const text = [
    asstTools({
      id: 'm1',
      blocks: [
        { type: 'tool_use', id: 't1', name: 'Bash', input: { command: 'git diff --exit-code' } },
      ],
    }),
    userResults({ blocks: [{ type: 'tool_result', tool_use_id: 't1', is_error: true }] }),
  ].join('\n');
...
test('waste: streaming re-writes of the same tool_use/result are counted once', () => {
  const line = asstTools({
    id: 'm1',
    blocks: [{ type: 'tool_use', id: 't1', name: 'Bash', input: { command: 'boom' } }],
  });
  const res = userResults({ blocks: [{ type: 'tool_result', tool_use_id: 't1', is_error: true }] });
  const retry = asstTools({
    id: 'm2',
    blocks: [{ type: 'tool_use', id: 't2', name: 'Bash', input: { command: 'boom' } }],
  });
...
test('waste: duplicate Read is redundant; a Read after an Edit is not', () => {
  const dup = parseSession(
    [
      asstTools({
        id: 'm1',
        blocks: [{ type: 'tool_use', id: 'r1', name: 'Read', input: { file_path: '/w/a.js' } }],
      }),
      asstTools({
        id: 'm2',
        blocks: [{ type: 'tool_use', id: 'r2', name: 'Read', input: { file_path: '/w/a.js' } }],
      }),
    ].join('\n'),
  ...
  const reset = parseSession(
    [
      ...
      asstTools({
        id: 'm2',
        blocks: [{ type: 'tool_use', id: 'e1', name: 'Edit', input: { file_path: '/w/a.js' } }],
      }),
      asstTools({
        id: 'm3',
        blocks: [{ type: 'tool_use', id: 'r2', name: 'Read', input: { file_path: '/w/a.js' } }],
  ...
  const s = parseSession(
    [
      ...
      asstTools({
        id: 'm2',
        blocks: [
          { type: 'tool_use', id: 'e1', name: 'MultiEdit', input: { file_path: '/w/a.js' } },
        ],
      }),
      asstTools({
        id: 'm3',
...
test('waste: a Read after a NotebookEdit on the same notebook path is not redundant', () => {
  const s = parseSession(
    [
      asstTools({
        id: 'm1',
        blocks: [{ type: 'tool_use', id: 'r1', name: 'Read', input: { file_path: '/w/nb.ipynb' } }],
      }),
      asstTools({
        id: 'm2',
        blocks: [
          {
            ...
            id: 'e1',
            name: 'NotebookEdit',
            input: { notebook_path: '/w/nb.ipynb' },
          },
        ],
      }),
      asstTools({
        id: 'm3',
        blocks: [{ type: 'tool_use', id: 'r2', name: 'Read', input: { file_path: '/w/nb.ipynb' } }],
      }),
    ].join('\n'),
        ...
        blocks: [
          {
            ...
            id: 'r1',
            name: 'Read',
            input: { file_path: '/w/big.log', offset: 0, limit: 2000 },
          },
        ],
      }),
      asstTools({
        ...
        blocks: [
          {
            ...
            id: 'r2',
            name: 'Read',
            input: { file_path: '/w/big.log', offset: 2000, limit: 2000 },
          },
        ],
      }),
      asstTools({
  ...
  const s = parseSession(
    [
      ...
      asstTools({
        id: 'm2',
        blocks: [
          { type: 'tool_use', id: 'b1', name: 'Bash', input: { command: 'sed -i s/x/y/ /w/a.js' } },
        ],
      }),
      asstTools({
        id: 'm3',
...
test('waste: a retried errored call keeps a sample with its command and error text', () => {
  const text = [
    asstTools({
      id: 'm1',
      blocks: [{ type: 'tool_use', id: 't1', name: 'Bash', input: { command: 'npm run boom' } }],
    }),
    userResults({
      blocks: [
        { type: 'tool_result', tool_use_id: 't1', is_error: true, content: 'File does not exist' },
    ...
    asstTools({
      id: 'm2',
      blocks: [{ type: 'tool_use', id: 't2', name: 'Bash', input: { command: 'npm run boom' } }],
    }), // retry
  ].join('\n');
...
test('waste: no sample is kept for an errored call that is never retried', () => {
  const text = [
    asstTools({
      id: 'm1',
      blocks: [{ type: 'tool_use', id: 't1', name: 'Bash', input: { command: 'ls /nope' } }],
    }),
    userResults({
      blocks: [
        { type: 'tool_result', tool_use_id: 't1', is_error: true, content: 'File does not exist' },
...
test('waste: error samples redact credentials and cap at 3 distinct per tool+reason', () => {
  ...
  for (let i = 0; i < 4; i++) {
        ...
        blocks: [
          {
            ...
            id: `t${i}`,
            name: 'Bash',
            input: { command: `curl --token deadbeef${i} tag${i}` },
          },
        ],
      }),
    );
    lines.push(
      userResults({
        blocks: [{ type: 'tool_result', tool_use_id: `t${i}`, is_error: true, content: 'boom' }],
      }),
    );
    lines.push(
      asstTools({
...
test('redactSecrets masks values but keeps surrounding text', () => {
  ...
  assert.strictEqual(
    redactSecrets('curl --token deadbeef https://x'),
    'curl --token «redacted» https://x',
  );
  assert.strictEqual(
    redactSecrets('-H "Authorization: Bearer xyz"'),
...
test('redactSecrets catches env-var style names where the keyword is glued to a prefix by _', () => {
  assert.strictEqual(
    redactSecrets('export DB_PASSWORD=hunter22222'),
    'export DB_PASSWORD=«redacted»',
  );
  assert.strictEqual(
    redactSecrets('export STRIPE_SECRET_KEY=sk_live_abcdefgh'),
    'export STRIPE_SECRET_KEY=«redacted»',
  );
  assert.strictEqual(
    redactSecrets('export MY_TOKEN=abcdefghijklmnop'),
...
test('mergeSessionAggregates merges waste across a main and a subagent file', () => {
  ...
  const sub = parseSession(
    [
      asstTools({
        id: 's1',
        blocks: [{ type: 'tool_use', id: 'u1', name: 'Bash', input: { command: 'boom' } }],
      }),
      userResults({ blocks: [{ type: 'tool_result', tool_use_id: 'u1', is_error: true }] }),
      asstTools({
        id: 's2',
        blocks: [{ type: 'tool_use', id: 'u2', name: 'Bash', input: { command: 'boom' } }],
      }), // retry of u1
    ].join('\n'),
    { sessionId: 's', project: 'p' },
  );
...
test('buildResponse aggregates waste and byProject across sessions in different projects', () => {
  ...
  assert.deepStrictEqual(r.waste.erroredByTool, [
    { name: 'Bash', count: 2 },
    { name: 'Edit', count: 1 },
  ]);
  ...
  const projB = parseSession(
    [
      ...
      userResults({
        ...
        blocks: [{ type: 'tool_result', tool_use_id: 'u1', is_error: true }],
      }),
      asstTools({
        id: 'b2',
...
test('classifyErrorReason: known Claude Code error strings map to specific reasons', () => {
  ...
  assert.strictEqual(
    classifyErrorReason(
      'File does not exist. Note: your current working directory is /Users/x/proj.',
    ),
    'file-not-found',
  );
  assert.strictEqual(
...
test('waste: erroredByReason is attributed at retry time and aggregated in buildResponse', () => {
  const s = parseSession(
    [
      asstTools({
        id: 'm1',
        blocks: [{ type: 'tool_use', id: 't1', name: 'Edit', input: { file_path: '/w/a.js' } }],
      }),
      userResults({
        blocks: [
          {
      ...
      asstTools({
        id: 'm2',
        blocks: [{ type: 'tool_use', id: 't2', name: 'Edit', input: { file_path: '/w/a.js' } }],
      }), // retry
    ].join('\n'),
